...
(
    ...
    role       varchar(255)
        constraint users_role_check
            check ((role)::text = ANY
        (ARRAY [('USER'::character varying)::text, ('ORGANIZER'::character varying)::text, ('ADMIN'::character varying)::text]))
    );
...
(
    ...
    id           uuid    not null
        primary key,
    organizer_id uuid
        constraint fkdocju8m76a3f8o6ljh2jrn2ra
            references users,
    venue_id     uuid
    ...
    status       varchar(255)
        ...
        (ARRAY [('DRAFT'::character varying)::text, ('PUBLISHED'::character varying)::text, ('CANCELLED'::character varying)::text, ('COMPLETED'::character varying)::text])),
    title        varchar(255)
);
...
(
    ...
    status     varchar(255)
        constraint bookings_status_check
            check ((status)::text = ANY
        (ARRAY [('PENDING_PAYMENT'::character varying)::text, ('CONFIRMED'::character varying)::text, ('CANCELLED'::character varying)::text]))
    );
...
(
    ...
    status         varchar(255)
        constraint tickets_status_check
            check ((status)::text = ANY
        (ARRAY [('AVAILABLE'::character varying)::text, ('CLAIMED'::character varying)::text, ('USED'::character varying)::text]))
    );
